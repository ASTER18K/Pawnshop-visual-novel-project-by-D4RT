...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("다이얼로그") action ShowMenu("history")
        ...
        textbutton _("크레딧") action ShowMenu("about")

        if renpy.variant("pc"):

            ## Help isn't necessary or relevant to mobile devices.
            textbutton _("조작 방법") action ShowMenu("help")

            ## The quit button is banned on iOS and unnecessary on Android.
            textbutton _("게임 종료") action Quit(confirm=not main_menu)
...
screen about():
    ...
    use game_menu(_("크레딧"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("이 게임은 {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only]. 버전의 비주얼 노벨 엔진을 기반으로 만들어졌습니다. \n\nGUI 한글화: 이동현(aster1sk)\n제작: D4RT GAMES \n  \n\n[renpy.license!t]")
...
screen file_slots(title):
    ...
    default page_name_value = FilePageNameInputValue(pattern=_("페이지 {}"), auto=_("자동 저장된 세이브"), quick=_("퀵 세이브"))
...
screen preferences():
    ...
    use game_menu(_("옵션"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "radio"
                    label _("사이드 롤백")
                    text _("대사 양 옆 여백을 클릭/터치하여 앞으로 롤백")
                    textbutton _("비활성화") action Preference("rollback side", "disable")
                    textbutton _("왼쪽") action Preference("rollback side", "left")
                    textbutton _("오른쪽") action Preference("rollback side", "right")
...
screen history():
    ...
    use game_menu(_("다이얼로그"), scroll=("vpgrid" if gui.history_height else "viewport")):
    ...
    use game_menu(_("조작 방법"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                ...
                textbutton _("게임 패드") action SetScreenVariable("device", "gamepad")
...
screen keyboard_help():
    ...
    hbox:
        label _("Enter")
        text _("페이지를 넘기거나 확인키로 사용합니다.")

    hbox:
        label _("Space")
        text _("페이지를 넘길 수 있습니다 (루트창에서는 조작 불가)")
    ...
    hbox:
        label _("Ctrl")
        text _("누르는 동안만 다이얼로그를 넘깁니다.")

    hbox:
        label _("Tab")
        text _("다이얼로그를 맨 뒤로 스킵합니다.")

    hbox:
        label _("Page Up")
        text _("다이얼로그를 앞으로 롤백합니다.")

    hbox:
        label _("Page Down")
        text _("다이얼로그를 뒤로 롤백합니다. (페이지 넘기는 기능이 아님!)")
    ...
    hbox:
        label "V"
        text _("글자 읽어주기 (접근성 관련 기능 포함, 불안정함.)")
...
screen gamepad_help():
    ...
    textbutton _("게임패드 보정") action GamepadCalibrate()
